        ...
        static void Worker(string combo)
        {
            try
            {
                ...
                })
                {
                    ...
                    string check = request.Post("https://discord.com/api/v8/auth/register", "{\"fingerprint\":\"793580565130641419.LGQ5IVlIkNTEQfpHbXcQLA2ABrM\",\"email\":\"" + credentials[0] + "\",\"username\":\"" + credentials[1] + "\",\"password\":\"rth21e98!fmPP\",\"invite\":null,\"consent\":true,\"date_of_birth\":\"1993-05-03\",\"gift_code_sku_id\":null,\"captcha_key\":null}", "application/json").ToString();

                    if (check.Contains("EMAIL_ALREADY_REGIS"))
                    {
                        Variables.Valid++;
                        Variables.Checked++;
                        Variables.cps++;
                    ...
                    else if (check.Contains("EMAIL_TYPE_INVALID_EMA") || check.Contains("\"captcha-required"))
                    {
                        Variables.Invalid++;
                        Variables.Checked++;
                        Variables.cps++;
